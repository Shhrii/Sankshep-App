...
const FetchPosts: React.FC<FetchPostsProps> = ({ navigation }) => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await fetch(
        'https://sankshep.app/wp-json/wp/v2/posts?_embed&categories=9'
      );
      if (!response.ok) {
      ...
      setPosts(shuffleArray(data)); // Shuffle the posts
      setIsLoading(false);
  ...
  const decodeHTML = (html: string): string => he.decode(html); // Decode HTML entities
  ...
  if (isLoading) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text style={styles.loaderText}>Ayurveda Unfolded, Anytime..</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
            ...
            <View style={styles.postContent}>
              ...
              {post.meta?.Source_Tag && post.meta?.Source_Link ? (
                <Text
                  ...
                  onPress={() => handleSourceClick(post.meta.Source_Link || '')}
                >
      ...
      <View style={styles.bottomNavigation}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('NonDoctorPage' as never)}>
          <Image style={styles.Icon} source={require('../assets/news.png')}/>
          <Text style={styles.navText}>News</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.navButton} onPress={() => navigation.navigate('Pollx' as never)}>
